...
ggsave("../output/Fig4.omega_curves/omega_distribution_plot.pdf", plot = distribution, width = 13, height = 4, dpi = 300)
...
```{r}
write.csv(omega_data_combined, file = "../output/Fig4.omega_curves/combined_omega.csv")
```
...
brassicales_range <- c(218, 265)
rosales_range <- c(218, 265)
...
ggplot(combined_data, aes(x = relative_pos, y = `dN/dS`, fill = color)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 2.5), breaks = seq(0, 2.5, by = 0.5)) + # Set y-axis limits and interval
...
ggsave("../output/Fig4.omega_curves/raw_omega_region_1.pdf", width = 6, height = 3)
...
```{r}
# Define ranges for each group
fagales_range <- c(432, 479)
brassicales_range <- c(433, 480)
rosales_range <- c(434, 481)
...
ggplot(combined_data, aes(x = relative_pos, y = `dN/dS`, fill = color)) +
  ...
  theme_classic() +
   theme(legend.position = "right",
        ...
        axis.title.y = element_text(size = 15, color = "black"),
        axis.title.x = element_blank())
ggsave("../output/Fig4.omega_curves/raw_omega_region_2.pdf",  width = 6, height = 3)
