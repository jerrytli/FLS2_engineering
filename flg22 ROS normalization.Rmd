...
```{r}
pc = read.csv("../R/Bubble plot data with relative ROS%_240121.csv", header = TRUE)
...
ggsave(filename = "../R/flg22 variants ROS normalization.pdf", width = 5.5, height = 6)
```


Grey bubble
```{r}
# Convert 'value' to numeric, handling NAs
pcm$value <- as.numeric(as.character(pcm$value))
...
xx <- ggplot(pcm, aes(x = Receptor, y = variable)) + 
  ...
  theme(
    ...
    axis.text.x = element_text(colour = "black", size = 14, face = "bold", angle = 45, vjust = 1, hjust = 1), 
...
ggsave(filename = "../R/flg22 variants ROS normalization.pdf", width = 5.4, height = 4.4)
