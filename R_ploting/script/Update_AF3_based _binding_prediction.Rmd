...
ordered_metrics <- names(sort(auc_values, decreasing = FALSE))
...
roc_plot <- ggplot(roc_data, aes(x = Specificity, y = Sensitivity, color = Metric)) +
  ...
  geom_line(data = data.frame(x = c(0, 1), y = c(0, 1)), aes(x, y), linetype = "dashed", color = "gray") +
  scale_color_manual(values = color_matrix) +
  annotate("text", x = 0.95, y = seq(0.1, 0.3, length.out = length(ordered_metrics)), 
           label = paste(ordered_metrics, "AUC:", round(auc_values[ordered_metrics], 2)),
           color = unname(color_matrix[ordered_metrics]), hjust = 1, size = 6) +
