...
# Assuming the data has columns 'Known_outcome' and 'ipTM flg22_FLS2'
data <- data %>%
...
if(any(shapiro_results$shapiro_p_value < 0.05)) {
  ...
  wilcox_test <- wilcox.test(`ipTM flg22_FLS2` ~ Known_outcome, data = data)
  p_value <- wilcox_test$p.value
} else {
  test_method <- "t.test"
  test_name <- "t-test"
  # Perform t-test
  t_test <- t.test(`ipTM flg22_FLS2` ~ Known_outcome, data = data)
...
ggplot(data, aes(x = Known_outcome, y = `ipTM flg22_FLS2`, fill = Known_outcome)) +
  ...
  geom_jitter(size = 1, alpha = 0.5, color = "black", width = 0.1, show.legend = FALSE) +  # Points with jitter
   annotate("text", x = 1.5, y = max(data$`ipTM flg22_FLS2`) + 0.1, 
  ...
  labs(title = "Raincloud Plot of ipTM flg22_FLS2 by Experimental Result",
       x = "Experimental Result",
       y = "ipTM flg22_FLS2") +
  my_ggplot_theme

ggsave("../output/af3/raincloud.pdf", width = 6, height = 6, dpi = 300)
...
# Combine ROC coordinates into a single data frame for ggplot
roc_data <- do.call(rbind, lapply(ordered_metrics, function(metric) {
  data.frame(
    Metric = metric,
    Specificity = 1 - roc_coords[[metric]]$specificity,
    Sensitivity = roc_coords[[metric]]$sensitivity
  )
}))
...
roc_plot <- ggplot(roc_data, aes(x = Specificity, y = Sensitivity, color = Metric)) +
  ...
  scale_color_manual(values = color_matrix, 
                     labels = paste(ordered_metrics, "(AUC:", round(auc_values[ordered_metrics], 3), ")")) +
...
# Save the plot
ggsave("../output/af3/ROC_multiple_metrics.pdf", plot = roc_plot, width = 6.5, height = 6, dpi = 300)
