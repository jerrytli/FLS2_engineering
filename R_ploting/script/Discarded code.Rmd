...
# Calculate the sliding window average
window_size <- 15
conservation_scores$Sliding_Average <- zoo::rollapply(conservation_scores$Conservation.Score, 
                                                      width = window_size, 
                                                      by = 1, 
                                                      FUN = mean, 
                                                      align = 'center', 
...
p <- ggplot(conservation_scores, aes(x = Position, y = Sliding_Average)) +
    ...
    geom_smooth(se = FALSE, method = "loess", span = 0.02) + # Add a smooth curve (change span for smoothness)
    ...
    labs(title = "Fagales",
         x = "Amino Acid Position",
         y = "Normalized Conservation Score")
...
# Create the plot with only a smooth line, and vertical lines at specified positions
p <- ggplot(conservation_scores, aes(x = Position, y = Conservation.Score)) +
  #geom_line(size = 1.1) + # Adjust this value to make the line thicker  
  geom_smooth(se = FALSE, method = "loess", span = 0.05) + # Smooth curve only
    geom_vline(xintercept = c(247, 391, 412, 439, 460), color = "red", linetype = "longdash") +
...
# Save the plot to a file
ggsave("../output/Updated_Conservation_score.pdf", plot = p, width = 15, height = 6, dpi = 300)
