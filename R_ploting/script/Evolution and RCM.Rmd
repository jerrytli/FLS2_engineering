...
scatter_plot <- ggplot(data, aes(x = Distance_Y, y = `dn/ds`)) +
  ...
  geom_point(
    ...
    shape = 21, size = 2.5, stroke = 0.01, color = "white", alpha = 0.5  # Thin white outline, semi-transparency
  ) +
  # Residues in range (dark outline, solid fill)
  geom_point(
    data = subset(data, has_outline),
    aes(fill = category),  # Map fill color only
    shape = 21, size = 2.5, stroke = 0.3, color = "black", alpha = 1  # Thicker dark outline, solid fill
  ) +
  # Horizontal line at dn/ds = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
...
marginal_plot <- ggplot() +
  # Density of dN.dS <=1 dots in gray
  geom_density(data = subset(data, `dn/ds` <= 1), aes(x = Distance_Z), fill = "gray", alpha = 0.3) +
...
# Save the plot
ggsave("../output/omega/combined_distances_plot_flg22.pdf", final_plot, width = 8, height = 6)
...
marginal_plot <- ggplot() +
  # Density of dN.dS <=1 dots in gray
  geom_density(data = subset(data, `dn/ds` <= 1), aes(x = Distance_Z), fill = "gray", alpha = 0.3) +
  # Density of dN/dS > 1 residues in orange
  geom_density(data = marginal_data, aes(x = Distance_Z), fill = "#de8e2a", alpha = 0.5) +
  # Align axes with scatter plot
...
# Save the plot
ggsave("../output/omega/combined_distances_plot_SERK3.pdf", final_plot, width = 8, height = 6)
...
scatter_plot <- ggplot(data, aes(x = Minimal_Distance, y = `dn/ds`)) +
  ...
  geom_point(
    aes(fill = category),  
    shape = 21, size = 2.7, stroke = 0.1, color = "white", alpha = 0.8
  ) +
  # Horizontal line at dN/dS = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
...
# Save the plot
ggsave("../output/omega/combined_distances_plot_flg22_SERK3.pdf", final_plot, width = 10, height = 6)
