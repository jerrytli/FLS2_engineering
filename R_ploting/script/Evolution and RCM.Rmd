...
scatter_plot <- ggplot(data, aes(x = Minimal_Distance, y = `dn/ds`)) +
  ...
  geom_point(
    aes(fill = category),  
    shape = 21, size = 1.5, stroke = 0.1, color = "white", alpha = 0.8
  ) +
  # Horizontal line at dN/dS = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  ...
  scale_fill_manual(
    values = c(
      "dN/dS <= 1" = "grey", 
      "Closest to flg22" = "deepskyblue4", 
      "Closest to SERK3" = "#de8e2a"
    ),
    name = NULL
  ) +
  labs(
...
marginal_plot <- ggplot() +
  ...
  # Density of dN/dS > 1 residues in blue
  geom_density(data = subset(data, `dn/ds` > 1), aes(x = Minimal_Distance), fill = "#75a44c", alpha = 0.5) +
  # Density of dN/dS <= 1 residues in a new color (e.g., dark red)
  geom_density(data = subset(data, `dn/ds` <= 1), aes(x = Minimal_Distance), fill = "gray", alpha = 0.5) +
  # Align axes with scatter plot
