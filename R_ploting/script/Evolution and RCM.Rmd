...
###QvFLS2 Calpha distance 
```{r}
# Load necessary libraries
library(ggplot2)

# Read the data from the CSV file
data <- read.csv("../input/omega/QvFLS2_minimal_distances.csv")

# Add a column to categorize the data for coloring
data$category <- ifelse(
  data$X_Residue %in% c(246, 390, 438, 459, 486), 
  "Highlighted Residues", 
  ifelse(data$dn.ds >= 1, "dn/ds >= 1", "dn/ds < 1")
)

# Create the scatter plot
ggplot(data, aes(x = Distance, y = dn.ds)) +
  # Points for dn/ds < 1
  geom_point(data = subset(data, category == "dn/ds < 1"), color = "grey", size = 1.5) +
  # Points for dn/ds >= 1
  geom_point(data = subset(data, category == "dn/ds >= 1"), color = "deepskyblue4", size = 1.5) +
  # Highlighted residues
  geom_point(data = subset(data, category == "Highlighted Residues"), color = "red", size = 2) +
  # Horizontal line at dn/ds = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  # Customize labels and titles
  labs(
    title = "Distance from flg22, CA",
    x = "MinimalDistance from flg22 (Å)",
    y = "dN/dS",
    color = NULL
  ) +
  # Additional styling
my_ggplot_theme
ggsave("../output/omega/qvfls2_distance_from_flg22_CA.pdf", width = 6, height = 4.5)

```

###QvFLS2 all atom distance 
```{r}
# Load necessary libraries
library(ggplot2)

# Read the data from the CSV file
data <- read.csv("../input/omega/QvFLS2_minimal_distances_all_atoms.csv")
...
# Add a column to determine whether the outline should be applied
data$has_outline <- ifelse(
  data$dn.ds > 1 & data$X_Residue >= 239 & data$X_Residue < 502,
  TRUE,
  FALSE
)
...
# Create the scatter plot
ggplot(data, aes(x = Distance, y = dn.ds)) +
  # Points without outline
  geom_point(
    data = subset(data, !has_outline), 
    aes(fill = category), 
    shape = 21, size = 1.8, stroke = 0.001 , color = "white" # stroke = 0 ensures no outline
  ) +
  # Points with outline
  geom_point(
    data = subset(data, has_outline), 
    aes(fill = category), 
    shape = 21, color = "black", size = 1.5, stroke = 0.5  # stroke > 0 adds a black outline
  ) +
  # Horizontal line at dn/ds = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  # Customize labels and titles
  scale_fill_manual(
    values = c(
      "dn/ds < 1" = "grey",
      "dn/ds >= 1" = "deepskyblue4",
      "Highlighted Residues" = "red"
    ),
    name = NULL
  ) +
  labs(
    title = "Distance from flg22, all atoms",
    x = "Minimal Distance from flg22 (Å)",
    y = "dN/dS"
  ) +
  # Additional styling
  my_ggplot_theme+
...
# Save the plot
ggsave("../output/omega/qvfls2_distance_from_flg22_all_atoms_corrected.pdf", width = 6, height = 4.5)


```

###QvFLS2 Ca distance from serk3
```{r}
# Load necessary libraries
# Load necessary libraries
library(ggplot2)
...
# Read the data from the CSV file
data <- read.csv("../input/omega/QvFLS2_minimal_distances_serk3.csv")
...
ggplot(data, aes(x = Distance, y = dn.ds)) +
  ...
  # Customize labels and titles
  labs(
    title = "Distance from SERK3, CA",
    x = "Minimal Distance from SERK3 (Å)",
    y = "dN/dS",
    color = NULL
  ) +
  # Additional styling
my_ggplot_theme
ggsave("../output/omega/qvfls2_distance_from_SERK3_CA.pdf", width = 6, height = 4.5)

```

###QvFLS2 distance from serk3, all atoms
```{r}
# Load necessary libraries
# Load necessary libraries
library(ggplot2)
...
# Read the data from the CSV file
data <- read.csv("../input/omega/QvFLS2_minimal_distances_serk3_all_atoms.csv")
...
my_ggplot_theme <- theme_classic() +
  theme(
    ...
    axis.ticks = element_line(size = 0.8),
    legend.position = c(0.66, 0.15),  # Position the legend inside the plot, above X-axis
...
ggplot(data, aes(x = Distance, y = dn.ds)) +
  ...
  # Customize labels and titles
  labs(
    title = "Distance from SERK3, all atoms",
    x = "Minimal Distance from SERK3 (Å)",
    y = "dN/dS",
    color = NULL
  ) +
  # Additional styling
  my_ggplot_theme 
ggsave("../output/omega/qvfls2_distance_from_SERK3_all_atoms.pdf", width = 6, height = 4.5)

```


###MdFLS2 all atom distance 
```{r}
# Load necessary libraries
library(ggplot2)

# Read the data from the CSV file
data <- read.csv("../input/omega/MdFLS2_minimal_distances_all_atoms.csv")

# Add a column to categorize the data for coloring
data$category <- ifelse(
  data$X_Residue %in% c(175, 247, 367,433, 460, 462), 
  "Highlighted Residues", 
  ifelse(data$dn.ds >= 1, "dn/ds >= 1", "dn/ds < 1")
)
...
# Add a column to determine whether the outline should be applied
data$has_outline <- ifelse(
  data$dn.ds > 1 & data$X_Residue >= 241 & data$X_Residue < 505,
  TRUE,
  FALSE
)
...
ggplot(data, aes(x = Distance, y = dn.ds)) +
  ...
  labs(
    title = "MdFLS2 Distance from flg22, all atoms",
    x = "Minimal Distance from flg22 (Å)",
    y = "dN/dS"
  ) +
  # Additional styling
...
# Save the plot
ggsave("../output/omega/Mdfls2_distance_from_flg22_all_atoms.pdf", width = 6, height = 4.5)


```

###AtFLS2 all atom distance 
```{r}
# Load necessary libraries
library(ggplot2)

# Read the data from the CSV file
data <- read.csv("../input/omega/AtFLS2_minimal_distances_from_all_atoms.csv")

# Add a column to categorize the data for coloring
data$category <- ifelse(
  data$X_Residue %in% c(248, 371, 417, 533), 
  "Highlighted Residues", 
  ifelse(data$dn.ds >= 1, "dn/ds >= 1", "dn/ds < 1")
)
...
ggplot(data, aes(x = Distance, y = dn.ds)) +
  ...
  labs(
    title = "AtFLS2 Distance from flg22, all atoms",
    x = "Minimal Distance from flg22 (Å)",
    y = "dN/dS"
  ) +
  # Additional styling
...
# Save the plot
ggsave("../output/omega/Atfls2_distance_from_flg22_all_atoms.pdf", width = 6, height = 4.5)
...
### Combined plot for flg22 distance
```{r}
# Load necessary libraries
library(ggplot2)
library(patchwork)  # For combining plots

# Define the receptor-specific parameters
highlighted_residues <- list(
  AtFLS2 = c(248, 371, 417, 533),
  MdFLS2 = c(175, 247, 367, 433, 460, 462),
  QvFLS2 = c(246, 390, 438, 459, 486)
)

# Mark residues from LRR 8 - 18
outline_ranges <- list(
  AtFLS2 = c(265, 528),
  MdFLS2 = c(241, 505),
  QvFLS2 = c(239, 503)
...
# Read the CSV files and add receptor-specific labels
files <- list(
  AtFLS2 = "../input/omega/AtFLS2_minimal_distances_from_all_atoms.csv",
  MdFLS2 = "../input/omega/MdFLS2_minimal_distances_all_atoms.csv",
  QvFLS2 = "../input/omega/QvFLS2_minimal_distances_from_all_atoms.csv"
)

data_list <- lapply(names(files), function(receptor) {
  # Read CSV without modifying column names
  df <- read.csv(files[[receptor]], check.names = FALSE)
  
  # Add receptor and derived columns
  df$Receptor <- receptor
  df$highlight <- ifelse(
    df$X_Residue %in% highlighted_residues[[receptor]],
    "Highlighted Residues",
    NA
  )
  df$has_outline <- with(df, X_Residue >= outline_ranges[[receptor]][1] &
                             X_Residue < outline_ranges[[receptor]][2] &
                             `dn/ds` > 1)
  df
})
...
data$category <- ifelse(
  !is.na(data$highlight),
  "Highlighted Residues",
  ifelse(data$`dn/ds` >= 1, "dn/ds >= 1", "dn/ds < 1")
)

# Filter data for marginal plot (all dN/dS > 1)
marginal_data <- subset(data, `dn/ds` > 1)

# Create the scatter plot
scatter_plot <- ggplot(data, aes(x = Distance_Y, y = `dn/ds`)) +
  # Residues not in range (white outline, semi-transparent fill)
  geom_point(
    data = subset(data, !has_outline),
    aes(fill = category),  # Map fill color only
    shape = 21, size = 1.8, stroke = 0.2, color = "white", alpha = 0.5  # Thin white outline, semi-transparency
  ) +
  # Residues in range (dark outline, solid fill)
  geom_point(
    data = subset(data, has_outline),
    aes(fill = category),  # Map fill color only
    shape = 21, size = 1.8, stroke = 0.5, color = "black", alpha = 1  # Thicker dark outline, solid fill
  ) +
  # Horizontal line at dn/ds = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  # Customize labels and fill colors
  scale_fill_manual(
    values = c(
      "dn/ds < 1" = "grey",
      "dn/ds >= 1" = "deepskyblue4",
  ...
  labs(
    x = "Minimal Distance from ligand (\u00c5)",
    y = "dN/dS"
  ) +
  # Apply original theme and axis settings
  my_ggplot_theme +
  theme(
...
# Create the marginal density plot with all data in gray and dN/dS > 1 in blue
marginal_plot <- ggplot() +
  # Density of all plotted dots in gray
  geom_density(data = data, aes(x = Distance_Y), fill = "gray", alpha = 0.5) +
  # Density of dN/dS > 1 residues in blue
  geom_density(data = marginal_data, aes(x = Distance_Y), fill = "deepskyblue4", alpha = 0.5) +
  # Align axes with scatter plot
  scale_x_continuous(expand = c(0, 0), limits = c(min(data$Distance_Y), max(data$Distance_Y))) +
  theme_void() +
  theme(
    plot.margin = margin(0, 0, 0, 0)  # Remove margins
  )

# Combine the marginal and scatter plots using patchwork
final_plot <- marginal_plot + scatter_plot +
  plot_layout(heights = c(1, 4))  # Adjust height ratio for marginal and scatter plots

# Save the plot
ggsave("../output/omega/combined_distances_plot_flg22.pdf", final_plot, width = 6, height = 4.5)


```

### Combined plot for SERK3 distance
```{r}
# Load necessary libraries
library(ggplot2)
library(patchwork)  # For combining plots
...
# Create the scatter plot
scatter_plot <- ggplot(data, aes(x = Distance_Z, y = `dn/ds`)) +
  # Residues not in range (white outline, semi-transparent fill)
  geom_point(
    data = subset(data, !has_outline),
    aes(fill = category),  # Map fill color only
...
marginal_plot <- ggplot() +
  # Density of all plotted dots in gray
  geom_density(data = data, aes(x = Distance_Z), fill = "gray", alpha = 0.3) +
  # Density of dN/dS > 1 residues in blue
  geom_density(data = marginal_data, aes(x = Distance_Z), fill = "deepskyblue4", alpha = 0.5) +
  # Align axes with scatter plot
  scale_x_continuous(expand = c(0, 0), limits = c(min(data$Distance_Z), max(data$Distance_Z))) +
  theme_void() +
  theme(
    plot.margin = margin(0, 0, 0, 0)  # Remove margins
  )
...
# Save the plot
ggsave("../output/omega/combined_distances_plot_SERK3.pdf", final_plot, width = 6, height = 4.5)

```


### Plot min of flg22 SERK3 distance
```{r}
# Load necessary libraries
library(ggplot2)
library(patchwork)  # For combining plots
...
data_list <- lapply(names(files), function(receptor) {
  ...
  # Determine minimal distance and closest molecule
  df$Minimal_Distance <- pmin(df$Distance_Y, df$Distance_Z, na.rm = TRUE)
  df$Closest_To <- ifelse(df$Distance_Y < df$Distance_Z, "flg22", "SERK3")
...
data$category <- ifelse(
  data$`dn/ds` <= 1, "dN/dS <= 1",
  ifelse(data$Closest_To == "flg22", "Closest to flg22", "Closest to SERK3")
)

# Filter data for marginal plot
marginal_data_higher <- subset(data, `dn/ds` > 1)
marginal_data_lower <- subset(data, `dn/ds` <= 1)

# Create the scatter plot
scatter_plot <- ggplot(data, aes(x = Minimal_Distance, y = `dn/ds`)) +
  # Plot all residues with different colors based on conditions
  geom_point(
    aes(fill = category),  
    shape = 21, size = 1.3, stroke = 0.2, color = "white", alpha = 0.7
  ) +
  # Horizontal line at dN/dS = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  ...
  scale_fill_manual(
    values = c(
      "dN/dS <= 1" = "grey", 
      "Closest to flg22" = "#ff474c", 
      "Closest to SERK3" = "#ffaf42"
    ),
    name = NULL
  ) +
  labs(
    x = "Minimal Distance to flg22 or SERK3 (\u00c5)",
    y = "dN/dS"
  ) +
  # Apply theme
  my_ggplot_theme +
...
# Create the marginal density plot with three distributions
marginal_plot <- ggplot() +
  # Density of all residues (background) in gray
  geom_density(data = data, aes(x = Minimal_Distance), fill = "gray", alpha = 0.3) +
  # Density of dN/dS > 1 residues in blue
  geom_density(data = subset(data, `dn/ds` > 1), aes(x = Minimal_Distance), fill = "deepskyblue4", alpha = 0.5) +
  # Density of dN/dS <= 1 residues in a new color (e.g., dark red)
  geom_density(data = subset(data, `dn/ds` <= 1), aes(x = Minimal_Distance), fill = "darkred", alpha = 0.5) +
  # Align axes with scatter plot
  scale_x_continuous(expand = c(0, 0), limits = c(min(data$Minimal_Distance), max(data$Minimal_Distance))) +
  theme_void() +
  theme(plot.margin = margin(0, 0, 0, 0))

# Combine marginal and scatter plots
final_plot <- marginal_plot + scatter_plot +
  plot_layout(heights = c(1, 4))  

# Save the plot
ggsave("../output/omega/combined_distances_plot_flg22_SERK3.pdf", final_plot, width = 8, height = 4.5)
