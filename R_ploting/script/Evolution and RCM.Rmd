...
scatter_plot <- ggplot(data, aes(x = Distance_Y, y = `dn/ds`)) +
  ...
  geom_point(
    data = subset(data, !has_outline), 
    aes(fill = category), 
    shape = 21, size = 1.8, stroke = 0.001, color = "white"
  ) +
  # Points with outline
  geom_point(
    data = subset(data, has_outline), 
    aes(fill = category), 
    shape = 21, color = "black", size = 1.5, stroke = 0.5
  ) +
  # Horizontal line at dn/ds = 1
  geom_hline(yintercept = 1, color = "red", linetype = "dashed", size = 1) +
  # Customize labels
...
# Save the plot
ggsave("../output/omega/combined_distances_plot.pdf", final_plot, width = 6, height = 4.5)
